...
const ScrambledTitle: React.FC<ScrambledTitleProps> = ({
  ...
  useEffect(() => {
    if (mounted && scramblerRef.current) {
      const displayText = hoverText || title;
      const phrases = [displayText];
...
const RainingLetters: React.FC<RainingLettersProps> = ({
  ...
  return (
    <div className="crt-monitor">
      <div className="crt-content relative w-full h-screen bg-black overflow-hidden">
        {/* Title and Content */}
        <div className="absolute inset-0 flex items-center justify-center z-20">
          <div className="text-center">
            <ScrambledTitle title={title} hoverText={hoverText} />
            {children}
          </div>
        </div>
